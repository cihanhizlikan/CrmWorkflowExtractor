...
public static class RunReport
{
    public static string Markdown(RunState state)
    {
        ...
        text.AppendLine(CultureInfo.InvariantCulture, $"- `{Name(RunPaths.MigrationPlanCsv)}` / `{Name(RunPaths.MigrationPlan)}` — çalışma sayfası: her iş akışı için bir satır, canlı süreçler başta.");
        text.AppendLine(CultureInfo.InvariantCulture, $"- `{Name(RunPaths.DataFootprint)}` / `{Name(RunPaths.DataFootprintCsv)}` / `{Name(RunPaths.DataCascadesCsv)}` — birden fazla iş akışının yazdığı {Count(state, "data.sharedFields")} alan; başka bir iş akışını başlatan {Count(state, "data.cascades")} yazma, {Count(state, "data.cascadePairs")} çift arasında.");
        text.AppendLine(CultureInfo.InvariantCulture, $"- `{Name(RunPaths.CallGraph)}` / `{Name(RunPaths.CallGraphCsv)}` — {Count(state, "callGraph.entryPoints")} giriş noktası; başka bir iş akışının çağırdığı {Count(state, "callGraph.buildingBlocks")} iş akışı.");
        text.AppendLine(CultureInfo.InvariantCulture, $"- `{Name(RunPaths.Usage)}` / `{Name(RunPaths.UsageCsv)}` — gruplamanın dışında tutulan {Count(state, "usage.drafts")} taslak tanım ve ürünle gelen {Count(state, "clusters.suppliedHeldApart")} iş akışı; adı deneme gibi okunan {Count(state, "usage.testNamedActive")} etkin iş akışı; {(state.StagesRun.Contains(RunStages.Usage) ? $"{Count(state, "usage.definitions")} tanımın {Count(state, "usage.withLoggedRun")} tanesinin çalıştığına dair kayıt var" : "kullanım kanıtı yok (Run:UsageFile)")}.");
