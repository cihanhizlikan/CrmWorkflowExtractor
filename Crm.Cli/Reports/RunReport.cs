...
public static class RunReport
{
    public static string Markdown(RunState state)
    {
        ...
        text.AppendLine(CultureInfo.InvariantCulture, $"- `data-footprint.md` / `.csv` / `data-cascades.csv` — {Count(state, "data.sharedFields")} field(s) written by more than one workflow; {Count(state, "data.cascades")} write(s) that start another workflow, between {Count(state, "data.cascadePairs")} pairs.");
        ...
        text.AppendLine(CultureInfo.InvariantCulture, $"- `usage.md` / `usage.csv` — {Count(state, "usage.drafts")} draft definition(s) and {Count(state, "clusters.suppliedHeldApart")} supplied with the product held apart from grouping; {Count(state, "usage.testNamedActive")} activated workflow(s) whose name reads like a test; {(state.StagesRun.Contains("usage") ? $"{Count(state, "usage.withLoggedRun")} of {Count(state, "usage.definitions")} definitions with a logged run" : "no usage evidence (Run:UsageFile)")}.");
