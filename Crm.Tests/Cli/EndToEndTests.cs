...
public sealed class EndToEndTests
{
    ...
    public async Task A_Clean_Run_Produces_A_Complete_Folder_Whose_Count_Chain_Balances()
    {
        ...
        Assert.Equal(5, Directory.GetFiles(Path.Combine(runRoot, "bpmn")).Length);
    ...
    public async Task Reprocessing_An_Earlier_Run_Rebuilds_Every_Offline_Output_Without_The_Network()
    {
        ...
        Assert.Equal(Directory.GetFiles(Path.Combine(firstRoot, "bpmn")).Select(Path.GetFileName), Directory.GetFiles(Path.Combine(secondRoot, "bpmn")).Select(Path.GetFileName));
