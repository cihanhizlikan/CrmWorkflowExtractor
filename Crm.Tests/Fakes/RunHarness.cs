...
{
    public static async Task<(ExitCode Code, string RunRoot, string Console)> RunAsync(FakeCrmServer server, TemporaryOutput output, bool requirePrivileges = true)
    {
        ...
        ExtractorSettings settings = new(
            ...
            Options.Create(new RunOptions { RequireOrganizationReadPrivileges = requirePrivileges }));
